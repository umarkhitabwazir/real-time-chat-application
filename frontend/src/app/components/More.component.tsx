...
const MoreComponent = ({currentUserName ,message,fetchMessages }: {currentUserName:string | null, message: Message ,fetchMessages:()=>Promise<void>}) => {
  ...
  return (
    <>
      <div onMouseLeave={() => { setShowComponent(true), setShowDelete(false) }} className={`${showComponent ? "hidden" : ''} relative right-3 z-48  bg-blue-500 shadow-2xl rounded-md flex justify-center items-start p-1 h-60 w-40`}>
        <button
